...
import {
  Users, GraduationCap, UserCog, UserCheck,
  School, BookOpen, Layers, TrendingUp,
  ClipboardList, Calendar, Bell,
} from "lucide-react";
...
import { DashboardHeader } from "@/components/dashboard/mainDashboard/Dashboardheader";
import { MiniStatCard, TopStatCard } from "@/components/dashboard/mainDashboard/Statecards";
import { EnrollmentByClass } from "@/components/dashboard/mainDashboard/Enrollmentbyclass";
import { EarningsCard } from "@/components/dashboard/mainDashboard/Earningscard";
import { GenderChart } from "@/components/dashboard/mainDashboard/Genderchart";
import { AttendanceChart } from "@/components/dashboard/mainDashboard/Attendancechart";
import { SchoolSummary } from "@/components/dashboard/mainDashboard/Schoolsummary";
import { LeaveRequests } from "@/components/dashboard/mainDashboard/Leaverequest";
import { RecentStudents } from "@/components/dashboard/mainDashboard/Recentstudents";
...
export default function DashboardPage() {
  ...
  const {
    ...
    totalExpenses, totalPayCount,
    avgEnrollPct, bestMonth,
    refresh,
  } = useDashboardData(primaryColor);
      ...
      <div className="max-w-screen-2xl mx-auto space-y-5">
        ...
        {error && (
          <div className="flex items-center gap-3 bg-rose-50 border border-rose-200 rounded px-4 py-3">
            ...
            <button
              onClick={refresh}
              className="text-[11px] font-bold text-rose-500 underline"
            >
              Retry
            </button>
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {topStats.map(s => (
            <TopStatCard key={s.label} {...s} loading={loading} />
          ))}
        </div>

        {/* Secondary 8 Mini Cards */}
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
          {secondaryStats.map(s => (
            <MiniStatCard key={s.label} {...s} loading={loading} />
          ))}
        </div>
