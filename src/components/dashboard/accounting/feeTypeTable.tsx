...
const FeeTypeTable = ({
  ...
  const [list, setList] = useState<FeeType[]>([]);
  ...
  const fetchFeeTypes = async () => {
    try {
      setLoading(true);
      const res = await FeeServices.getAllFeeTypes();
  ...
  const downloadPDF = () => {
    const doc = new jsPDF();
    doc.setFontSize(16);
    doc.text("Fee Types Report", 14, 15);
    doc.setFontSize(10);
    ...
    autoTable(doc, {
      head: [["S.N.", "Fee Type Name", "Code"]],
      body: tableData,
      startY: 28,
      styles: { fontSize: 9 },
      headStyles: { fillColor: [54, 74, 99] },
    });
    doc.save(`Fee_Types_Report_Page_${currentPage}.pdf`);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      await Promise.all(ids.map((id) => FeeServices.deleteFeeType(id)));
    ...
    <div className="space-y-3 font-mukta">
          ...
          <table className="w-full text-left border-separate border-spacing-0">
            <thead className="sticky top-0 z-30 shadow-sm">
              <tr className="bg-[#f5f6fa]">
                ...
                <th className="px-6 py-2 text-[11px] font-bold text-[#8094ae] uppercase border-b">Fee Type Info</th>
                <th className="px-6 py-2 text-[11px] font-bold text-[#8094ae] uppercase border-b">Unique Code</th>
                  ...
                  <td colSpan={5} className="text-center py-16">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <span className="text-sm font-bold text-[#364a63]">
                        {searchQuery ? "No matching types found." : "No fee types defined."}
                      </span>
                    </div>
                  </td>
                </tr>
                ...
                paginatedItems.map((item, index) => {
                  ...
                  return (
                    <tr key={item.id} className={`hover:bg-gray-50 transition-colors ${isSelected ? "bg-blue-50/40" : ""}`}>
                      ...
                      <td className="px-6 py-2">
                        <div className="flex items-center gap-1.5 text-[11px] font-mono font-medium text-slate-500 bg-slate-50 w-fit px-2 py-0.5 rounded border border-slate-100">
                          <Hash size={10} /> {item.code}
                        </div>
                      </td>
      ...
      {selectedIds.length > 0 && (
        <div className="flex items-center justify-between animate-in fade-in slide-in-from-bottom-2 px-1">
          <span className="text-[10px] font-bold text-red-600 uppercase tracking-widest">{selectedIds.length} Types Selected</span>
          <button 
      ...
      <ConfirmModal
        isOpen={isModalOpen}
        title={selectedIds.length > 0 ? "Delete Selected Types?" : "Remove Fee Type?"}
        message={selectedIds.length > 0 ? `Are you sure you want to delete ${selectedIds.length} fee types?` : "Deleting this fee type might affect existing fee structures. Are you sure?"}
