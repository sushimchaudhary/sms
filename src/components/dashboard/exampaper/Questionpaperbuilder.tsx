...
export default function QuestionPaperBuilder({ initialData }: BuilderProps) {
  ...
  // ── Track which sections have had auto-open fired ─────────────────────────
  const autoOpenedRef = React.useRef<Set<number>>(new Set());
  ...
  // ── Auto-open blank question form when entering step 3 or switching section ──
  // Fires once per section index (tracked via autoOpenedRef) when question count is 0

  useEffect(() => {
    if (step !== 3) return;
    const questions = questionsBySection[activeSectionIdx] ?? [];
    const hasUnsaved = questions.some((q) => !q.saved);

    // Only auto-open if: section has no questions at all AND no form exists AND not already fired
    if (
      questions.length === 0 &&
      !hasUnsaved &&
      !autoOpenedRef.current.has(activeSectionIdx)
    ) {
      autoOpenedRef.current.add(activeSectionIdx);
      setQuestionsBySection((prev) => ({
        ...prev,
        [activeSectionIdx]: [
          {
  ...
  }, [step, activeSectionIdx]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const removeSection = (idx: number) => {
    ...
    // Also clean up the auto-open tracker for shifted indices
    const newSet = new Set<number>();
    autoOpenedRef.current.forEach((i) => {
      if (i < idx) newSet.add(i);
      else if (i > idx) newSet.add(i - 1);
    });
    autoOpenedRef.current = newSet;
    setActiveSectionIdx((prev) => Math.max(0, prev > idx ? prev - 1 : prev));
  };
  ...
  const addQuestion = (sectionIdx: number) => {
    const existing = questionsForSection(sectionIdx);
    // Block if there's already an unsaved (non-saved) form open
    const hasUnsaved = existing.some((q) => !q.saved);
    if (hasUnsaved) {
      toast.error("Save the current question before adding a new one.");
      return;
    }
    setQuestionsBySection((prev) => ({
      ...prev,
  ...
  const saveQuestion = async (sectionIdx: number, qIdx: number) => {
    ...
    try {
      ...
      // Step 1 — mark saved so the card collapses with green tick
      setQuestionsBySection((prev) => {
        const list = [...(prev[sectionIdx] ?? [])];
        list[qIdx] = { ...list[qIdx], id: res?.id, saved: true };
        return { ...prev, [sectionIdx]: list };
      ...
      // Step 2 — after brief pause, append a fresh blank form (expanded by default)
      const nextOrder = (q.order ?? 1) + 1;
      setTimeout(() => {
        setQuestionsBySection((prev) => {
          const currentList = prev[sectionIdx] ?? [];
          // Only append if no other unsaved form exists
          const alreadyHasUnsaved = currentList.some((item) => !item.saved);
          if (alreadyHasUnsaved) return prev;
          return {
            ...prev,
            [sectionIdx]: [
              ...currentList,
                ...
                <div className="w-36 flex-shrink-0 space-y-1">
                  {sections.map((sec, si) => (
                    ...
                    >
                      Section {sec.title}
                      <span className={`ml-1 text-[10px] ${activeSectionIdx === si ? "opacity-60" : "text-gray-400"}`}>
                        ({questionsForSection(si).filter((q) => q.saved).length}q)
                      </span>
                    </button>
                  ))}
                </div>
...
function QuestionCard({
  ...
  // Collapse automatically when the card gets marked as saved
  useEffect(() => {
    if (q.saved) setExpanded(false);
  }, [q.saved]);
