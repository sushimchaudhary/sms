...
const TeachersTable = ({
  ...
  const fetchTeachers = async () => {
    try {
      ...
      const allTeachers = Array.isArray(res) ? res : res?.results || res?.data || [];
      setTeachersList([...allTeachers].reverse());
    } catch (error) {
  ...
  useEffect(() => {
    let result = teachersList.filter((t) => {
      const name = (t.user?.full_name || t.full_name || "").toLowerCase();
  ...
  const downloadPDF = () => {
    const doc = new jsPDF();
    doc.setFontSize(16);
    doc.text("Teachers List Report", 14, 15);
    doc.setFontSize(10);
    ...
    doc.save(`Teachers_Report_Page_${currentPage}.pdf`);
    ...
    if (printWindow) {
      printWindow.document.write(`
        <html>
          ...
          <body>
            <h2>Teachers List</h2>
            <div class="meta">Generated for Page ${currentPage} | Total Teachers: ${filteredData.length}</div>
            <table>
              <thead>
                <tr>
                  <th>S.N.</th>
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      await Promise.all(idsToDelete.map((id) => TeacherServices.deleteTeacher(id)));
    ...
    <div className="space-y-3">
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                  ...
                  <td colSpan={6} className="text-center py-16">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <span className="text-sm font-bold text-[#364a63]">
                        {searchQuery ? "No matching teachers found." : "No teachers registered."}
                      </span>
                    </div>
                  </td>
                </tr>
                ...
                paginatedItems.map((item, index) => {
                  ...
                  const teacherPhoto = item.photo_url || item.photo;
                      ...
                      <td className="px-6 py-1">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="relative">
                             <Avatar 
                                src={teacherPhoto} 
                                icon={!teacherPhoto && <User size={14} />} 
                                size={32} 
                                className="border border-gray-100 shadow-sm shrink-0"
                             />
                          </div>
                          <div className="flex flex-col">
                            <span className="text-[11px] text-[#364a63] font-bold uppercase leading-tight">
                      ...
                      <td className="px-6 py-1">
                        <div className="flex flex-col gap-1">
                          ...
                          <div className="flex items-center gap-2 text-[#526484]">
                            ...
                            <span className="text-[10px] italic">{item.qualification || "Not Specified"}</span>
                          </div>
                        </div>
                      </td>
      ...
      {selectedIds.length > 0 && (
        <div className="flex items-center justify-between animate-in fade-in slide-in-from-bottom-2">
          <span className="text-xs font-bold text-red-600 uppercase tracking-wider">{selectedIds.length} Teachers Selected</span>
      ...
      <ConfirmModal
        isOpen={isModalOpen}
        title={selectedIds.length > 0 ? "Delete Selected Teachers?" : "Remove Teacher?"}
        message={selectedIds.length > 0 ? `Are you sure you want to delete ${selectedIds.length} teachers?` : "Are you sure you want to delete this profile?"}
