...
export type PrimarySkin  =
  | "default" | "bluelight" | "egyptian" | "purple" | "blue" | "red"
  | "orange" | "pink" | "cyan" | "yellow" | "custom";
...
export interface SettingsDrawerProps {
  ...
  skinMode: SkinMode;           setSkinMode: (v: SkinMode) => void;
  topbarColor: TopbarColor;     setTopbarColor: (v: TopbarColor) => void;
  menuColor: MenuColor;         setMenuColor: (v: MenuColor) => void;
  sidebarSize: SidebarSize;     setSidebarSize: (v: SidebarSize) => void;
  direction: Direction;         setDirection: (v: Direction) => void;
  uiStyle: UiStyle;             setUiStyle: (v: UiStyle) => void;
  sidebarStyle: SidebarStyle;   setSidebarStyle: (v: SidebarStyle) => void;
  navStyle: NavStyle;           setNavStyle: (v: NavStyle) => void;
  primarySkin: PrimarySkin;     setPrimarySkin: (v: PrimarySkin) => void;
  // custom color picker
...
function PreviewLight({ active }: { active: boolean }) {
  return (
    <svg viewBox="0 0 80 60" className="w-full h-full">
      ...
      {active && <text x="68" y="55" textAnchor="middle" fontSize="6" fill="white">✓</text>}
    </svg>
  );
}
...
function PreviewTopbar({ color, activeColor, active }: {
  color: "light" | "dark" | "theme"; activeColor: string; active: boolean;
}) {
  const topbarFill = color === "light" ? "#ffffff" : color === "dark" ? "#1f2937" : activeColor;
  return (
    <svg viewBox="0 0 80 60" className="w-full h-full">
...
function PreviewMenuColor({ color, brandColor, active }: {
  color: "light" | "dark" | "brand"; brandColor: string; active: boolean;
}) {
  const sidebarFill = color === "light" ? "#e5e7eb" : color === "dark" ? "#1e293b" : brandColor;
  const dotFill     = color === "light" ? "#9ca3af" : color === "dark" ? "#475569" : "rgba(255,255,255,0.5)";
  const logoBg      = color === "light" ? "#cbd5e1" : color === "dark" ? "#334155" : "rgba(255,255,255,0.25)";
  return (
    <svg viewBox="0 0 80 64" className="w-full h-full">
      ...
      {active && <text x="70" y="59" textAnchor="middle" fontSize="6" fill="white">✓</text>}
    </svg>
  );
}

function PreviewSidebarSize({ size, active }: {
      ...
      {isFullscreen ? (
        <>
          ...
          {[10, 20, 30, 40, 50].map((y) => (
            <rect key={y} x="8" y={y} width="64" height="4" fill="#475569" rx="1" />
          ))}
        </>
      ) : (
        <>
          ...
          {size === "condensed" || size === "hover"
            ? [10, 20, 30, 40, 50].map((y) => (
                <rect key={y} x="2" y={y} width="4" height="4" fill="#64748b" rx="1" />
              ))
            : [10, 20, 30, 40, 50].map((y) => (
                <rect key={y} x="3" y={y} width={sw - 5} height="3" fill="#64748b" rx="1" />
              ))}
          {isHover && (
            <>
              <rect x={sw} y="0" width="18" height="64" fill="#1e293b" opacity="0.92" />
              {[10, 20, 30, 40, 50].map((y) => (
                <rect key={y} x={sw + 2} y={y} width="14" height="3" fill="#64748b" rx="1" />
              ))}
            </>
          )}
          ...
          {size !== "full" && (
            <>
              <rect x={sw + 3} y="13" width={(80 - sw - 6) / 2 - 1} height="14" fill="#e2e8f0" rx="2" />
              <rect x={sw + 3 + (80 - sw - 6) / 2 + 1} y="13" width={(80 - sw - 6) / 2 - 1} height="14" fill="#e2e8f0" rx="2" />
              <rect x={sw + 3} y="30" width={80 - sw - 6} height="7" fill="#e2e8f0" rx="2" />
              <rect x={sw + 3} y="40" width={80 - sw - 6} height="7" fill="#e2e8f0" rx="2" />
              <rect x={sw + 3} y="50" width={80 - sw - 6} height="7" fill="#e2e8f0" rx="2" />
            </>
          )}
          {size === "full" && (
            <>
              <rect x={sw + 3} y="13" width={80 - sw - 6} height="8" fill="#e2e8f0" rx="2" />
              <rect x={sw + 3} y="24" width={80 - sw - 6} height="8" fill="#e2e8f0" rx="2" />
              <rect x={sw + 3} y="35" width={80 - sw - 6} height="8" fill="#e2e8f0" rx="2" />
            </>
          )}
        </>
      )}
...
export default function SettingsDrawer({
  ...
  const closePassModal = () => {
    setIsClosing(true);
    setTimeout(() => { setIsPassModalOpen(false); setIsClosing(false); }, 300);
  };
  ...
  const primarySkins = [
    { key: "default",   label: "Default",   gradient: "from-emerald-400 to-teal-500"  },
    { key: "bluelight", label: "Blue Light", gradient: "from-blue-300 to-blue-400"     },
    { key: "egyptian",  label: "Egyptian",   gradient: "from-teal-600 to-[#0f4c3a]"   },
    { key: "purple",    label: "Purple",     gradient: "from-purple-400 to-violet-600" },
    { key: "blue",      label: "Blue",       gradient: "from-blue-400 to-blue-600"     },
    { key: "red",       label: "Red",        gradient: "from-red-400 to-rose-500"      },
    { key: "orange",    label: "Orange",     gradient: "from-orange-400 to-orange-600" },
    { key: "pink",      label: "Pink",       gradient: "from-pink-400 to-rose-400"     },
    { key: "cyan",      label: "Cyan",       gradient: "from-cyan-400 to-sky-500"      },
    { key: "yellow",    label: "Yellow",     gradient: "from-yellow-400 to-amber-500"  },
  ] as const;
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          ...
          <button onClick={onClose} className="p-1 hover:bg-gray-100 rounded text-slate-400">
            <X size={18} />
          </button>
        </div>
        ...
        <div className="p-5 space-y-6 overflow-y-auto h-full pb-24 text-xs">
              ...
              {(["light", "dark", "theme"] as const).map((color) => (
                <div key={color} className="flex flex-col items-center gap-1.5">
                  <button
                    ...
                    className={`w-full aspect-[4/3] rounded-lg border-2 overflow-hidden transition-all ${
                      topbarColor === color ? "border-indigo-500" : "border-gray-200"
                    }`}
                  >
                    <PreviewTopbar color={color} activeColor={activeColor} active={topbarColor === color} />
                  </button>
                  <span className="text-[11px] text-slate-500 font-medium capitalize">{color}</span>
                </div>
              ))}
            </div>
          </div>
              ...
              {(["light", "dark", "brand"] as const).map((color) => (
                <div key={color} className="flex flex-col items-center gap-1.5">
                  <button
                    ...
                    className={`w-full aspect-[4/3] rounded-lg border-2 overflow-hidden transition-all ${
                      menuColor === color ? "border-indigo-500" : "border-gray-200"
                    }`}
                  >
                    <PreviewMenuColor color={color} brandColor={activeColor} active={menuColor === color} />
                  </button>
                  <span className="text-[11px] text-slate-500 font-medium capitalize">{color}</span>
                </div>
              ))}
              ...
              {([
                ...
                <div key={key} className="flex flex-col items-center gap-1.5">
                  <button
                    ...
                    className={`w-full aspect-[4/3] rounded-lg border-2 overflow-hidden transition-all ${
                      sidebarSize === key ? "border-indigo-500" : "border-gray-200"
                    }`}
                  >
                    <PreviewSidebarSize size={key} active={sidebarSize === key} />
                  </button>
          ...
          <div>
            ...
            <div className="grid grid-cols-4 gap-2">
              {[
                { key: "white", cls: "bg-white border border-gray-300", label: "White" },
                ...
                <div key={key} className="flex flex-col items-center gap-1">
                  <button
                    ...
                    style={key === "theme" ? { backgroundColor: activeColor } : {}}
                  >
                    {sidebarStyle === key && (
          ...
          <div>
            ...
            <div className="grid grid-cols-4 gap-2">
              {[
                { key: "white", cls: "bg-white border border-gray-300",     label: "White" },
                { key: "light", cls: "bg-slate-100 border border-gray-200", label: "Light" },
                ...
                <div key={key} className="flex flex-col items-center gap-1">
                  ...
                  >
                    {navStyle === key && (
                      <span className="absolute inset-0 flex items-center justify-center text-slate-700 text-xs">✓</span>
                    )}
                  </button>
                  <span className="text-[10px] text-slate-500">{label}</span>
                </div>
            ...
            <div className="mt-3 pt-3 border-t border-gray-100">
              ...
              <div
                ...
                className={`flex items-center gap-3 px-3 py-2 rounded-lg border-2 cursor-pointer transition-all hover:border-slate-300 ${
                  primarySkin === "custom" ? "border-emerald-500 bg-emerald-50/30" : "border-gray-200"
                }`}
              >
          ...
          <div className="pt-3 mt-2 border-t border-gray-100 flex flex-col gap-1">
            ...
            >
              <div className="p-1 rounded bg-slate-100 group-hover:bg-blue-100 transition-colors">
                <RotateCcw size={14} className="group-hover:rotate-[-45deg] transition-transform duration-300" />
              </div>
              Reset Settings
            </button>
            ...
            <button className="flex items-center gap-2.5 px-3 py-2 rounded-md text-[13px] font-semibold text-rose-600 hover:bg-rose-50 transition-all group">
      ...
      {isPassModalOpen && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
          <div
            className={`absolute inset-0 bg-slate-900/40 backdrop-blur-sm transition-opacity duration-300 ${isClosing ? "opacity-0" : "opacity-100"}`}
            onClick={closePassModal}
          />
          <div
            className={`relative w-full max-w-md transition-all duration-300 transform ${isClosing ? "scale-95 opacity-0" : "scale-100 opacity-100 animate-in zoom-in-95"}`}
          >
            <ChangePassword onClose={closePassModal} />
          </div>
        </div>
      )}
    </>
